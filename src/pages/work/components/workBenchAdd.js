...
const AddWorkBench = props => {
    const {visible,setVisible, callBack, edit, match} = props;
    ...
    const onFinish = values => {
        if (edit) {
            WorkService.updateWork({ ...values, id: edit.id },match.params.tenant).then(res => {
                if (!res.code) {
                    message.success('成功!');
                    callBack()
                    setVisible(false)
            ...
            WorkService.createWorkAppLink(values,match.params.tenant).then(res => {
                if (!res.code) {
                    message.success('成功!');
                    callBack()
                    setVisible(false)
