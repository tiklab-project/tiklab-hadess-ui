/**
 * @name: dmRoleDetailPromise
 * @author: mahai
 * @date: 2021-05-06 15:19
 * @description：权限中心 项目域下的角色权限
...
const DomainRolePromise = props => {
    ...
    const getAllFunction = () => {
        Axios.post('/prjFunction/findPrjFunctionList', {
            group
            ...
            if (!res.code) {
                ...
                getRoleByPromise(roleDetail.roleId, true, data.result, data.resultChild)
            }
        })
    }
    ...
    /**
     * 处理已有权限的角色和没有权限的角色在checkbox进行标记
     * @param id 角色id
     * @param init 是否是初始加载
     * @param data 数据
     * @param dataGroup 子集数据
     */
    const getRoleByPromise = (id, init=false, data=[], dataGroup={}) => {
        const params = {
            roleId: id
        }
        Axios.post('prjRoleFunction/findPrjRoleFunctionList', params, match.params.tenant).then(res => {
            if (!res.code) {
    ...
    const addPromise = (id, flag=true) => {
        ...
        Axios.post('prjRoleFunction/createPrjRoleFunction', postParams, match.params.tenant).then(res => {
            if(flag) getRoleByPromise(roleDetail.roleId)
        })
    }
    // TODO 删除权限
    const deletePromise = (id,flag=true) => {
        ...
        Axios.post('prjRoleFunction/deletePrjRoleFunction', formData, match.params.tenant).then(res => {
            if(flag) getRoleByPromise(roleDetail.roleId)
        })
    }
    return(
                ...
                checkBoxData.map(item => {
                    return (
                        <div key={item.value}>
                            ...
                            <br/>
                            &nbsp; &nbsp; &nbsp; &nbsp;
                            <CheckboxGroup options={checkBoxGroup[item.value]} value={item.checkedList} onChange={(list) => onChange(item.value, list)} />
                        </div>
                    )
                })
            }
...
export default DomainRolePromise
